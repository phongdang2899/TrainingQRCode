...
class UnblockOTPMiddleware
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        $phoneNumber = $customer->checkPhoneEndUser($request->phone_number);
        if(empty($phoneNumber)){
            $data = [
                'message' => trans('message.txt_not_found_please_create_new_customer', ['attribute' => trans('message.phone_number')]),
                'error_code' => config('constants.error_code.customer_not_found')
            ];
            return new CustomerPhoneErrorResource($data);
        }
        $otpTracking = OtpTracking::where('ip', $ip)->where('customer_id', $phoneNumber->id)->first();
